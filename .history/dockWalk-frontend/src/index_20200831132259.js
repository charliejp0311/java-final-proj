...
let displayDocks = (docks) => {
    ...
    for (const section of d) {
        ...
        for (const range of sectionsRanges) {
            ...
            for (const p of prts) {
                drs.push(new Door(section.attributes.section,range.attributes.range,range.racks,p.attributes.number,p.attributes.stage,p.attributes.serviceable, p.attributes.service_comments))
            }
        }
