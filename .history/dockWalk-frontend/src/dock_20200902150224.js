    ...
    for (const section of d) {
        ...
        for (const range of sectionsRanges) {
            ...
            for (const p of prts) {
                ...
                ranges[ranges['length']-1].doors.push(drs[drs.length - 1])
            }
            dks[dks.length-1].ranges.push(ranges[ranges.length-1])
        }
        dockCard(dks[dks.length - 1], dks[dks.length - 1].ranges)
    }
    const docks = document.getElementById('dock')
    for (const dock of dks) {
        docks.appendChild(dispDk(dock))
    }
}
